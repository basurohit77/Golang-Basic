...
func (alphaReader AlphaReader) Read(p []byte) (int, error) {
	inputData := make([]byte, len(p)) // buffer created
	count, err := alphaReader.Src.Read(inputData) // pass pointer of buffer
	println(count)
	if err != nil {
		return count, err
	}
